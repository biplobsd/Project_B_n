
-- Region config
-- BTC spinner
LCheck = Region(228, 93, 207, 87)
wlc = Region(229, 1273, 455, 285)
spin = Region(762, 1709, 62, 22)
conE = Region(415, 771, 219, 93)
findspintre = Region(145, 581, 69, 79)
verifybtcapp = Region(896, 69, 128,132)
applist = Region(129, 437, 857, 1153)
rp1 = Region(643, 261, 97, 81)
rpc2 = Region(547, 292, 232, 47)
last_app = Region(420, 1318, 106, 59)
waitf = Region(225, 1257, 461, 329)
LCheck = Region(228, 93, 207, 87)
offer1 = Region(137, 495, 45, 57)
spin = Region(692, 1680, 130, 51)
spinColG = Region(811, 1690, 0, 0)
waitColG = Region(806, 1691, 0, 0)
lerrorR = Region(595, 845, 183, 75)
climeS = Region(286, 807, 284, 64)
climeC = Region(131, 755, 171, 71)
climeSuc = Region(297, 793, 259, 57)
PleWait = Region(163, 763, 757, 99)
verifybtcapp = Region(896, 69, 128,132)
lowPowerD = Region(273, 122, 0, 0)
unfortunatelyStoped = Region(199, 1789, 163, 65)
waitForApp = Region(43, 995, 0, 0)

-- Via app
homeBTCwA = Region(35, 419, 525, 95)
waitX = Region(976, 95, 83, 75)
topCoinBase = Region(115, 105, 247, 65)
verifyBTCw = Region(487, 465, 111, 111)
sendN = Region(189, 831, 197, 75)
sendNC = Region(219, 875, 1, 1)
sendBtcEmail = Region(19, 417, 1039, 101)
sendBtcE = Region(348, 758, 147, 60)
amount = Region(61, 645, 179, 63)
conticol = Region(512, 1584, 0, 0)
conticol2 = Region(711, 1489, 0, 0)
transD = Region(37, 353, 433, 253)
verifyviaapp = Region(497, 1821, 85, 79)
rp = Region(317, 303, 443, 77)
homeforWC = Region(166, 446, 0, 0)
rp1 = Region(643, 261, 97, 81)

-- Vpn Fixing
PinRequT = Region(125, 545, 293, 405)
passwordRequP = Region(129, 743, 281, 511)
connectingC = Region(420, 317, 69, 35)
vpnClientProC = Region(210, 130, 128, 57)
chromeC = Region(979, 119, 36, 75)
chromePlus = Region(54, 129, 59, 53)
webchromeH = Region(389, 745, 48, 37)
chromeLoading = Region(27, 237, 0, 0)
chromeAddressBar = Region(15, 93, 1057, 139)
numberchecking = Region(24, 270, 226, 38)
chromeDownloadAgain = Region(47, 1551, 73, 71)
sciweaversProg = Region(300, 713, 56, 56)
downloadtextfile = Region(88, 594, 51, 54)
ducumentsReg = Region(119, 1585, 873, 43)
tabswichingshort = Region(67, 281, 89, 1285)
chromeDownloadImg = Region(77, 429, 937, 1169)
cepbepy = Region(439, 707, 191, 59)
ducumentsRecent = Region(141, 285, 281, 107)
ducumentsUsa = Region(201, 293, 749, 61)
ducumentsLoad = Region(939, 269, 113, 105)
